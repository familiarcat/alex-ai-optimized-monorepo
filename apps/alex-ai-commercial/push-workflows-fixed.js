...
/**
 * Fixed n8n Workflow Push Script
 * Pushes workflows with correct n8n API format
 */
...
// Create simplified workflow format that n8n API accepts
function createSimplifiedWorkflow(name, webhookPath) {
    return {
        name: name,
        nodes: [
            {
...
async function pushWorkflowsToN8N() {
    console.log('🚀 Pushing Workflows to n8n.pbradygeorgen.com (Fixed Format)')
    console.log('============================================================')
    console.log('')
    ...
    try {
        ...
        // Step 2: Create simplified workflows
        console.log('📋 Creating simplified workflows...')
        const workflows = [
            {
        ...
        console.log('✅ Created 4 simplified workflows')
        console.log('')
        ...
        for (const workflowConfig of workflows) {
            try {
                ...
                const workflowData = createSimplifiedWorkflow(workflowConfig.name, workflowConfig.webhookPath)
        ...
        if (successCount > 0) {
            ...
            pushedWorkflows.filter(w => w.status === 'pushed').forEach(workflow => {
                console.log(`   https://n8n.pbradygeorgen.com/workflow/${workflow.id}`)
            })
        } else {
