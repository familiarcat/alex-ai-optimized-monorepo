...
function PaymentForm({ tier, price, onSuccess, onError }) {
    ...
    return (<form onSubmit={handleSubmit} className="space-y-6">
      ...
      <button type="submit" disabled={!stripe || loading} className="w-full flex justify-center items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed">
        {loading ? (<div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2"></div>) : (<outline_1.CreditCardIcon className="h-4 w-4 mr-2"/>)}
        {loading ? 'Processing...' : `Subscribe for $${price}/month`}
      </button>

      <div className="flex items-center justify-center text-xs text-gray-500">
        <outline_1.LockClosedIcon className="h-4 w-4 mr-1"/>
        Secure payment powered by Stripe
      </div>
    </form>);
}
function PaymentProcessor({ tier, price, onSuccess, onError }) {
    return (<div className="bg-white rounded-lg shadow-lg p-8 max-w-md mx-auto">
      ...
      <react_stripe_js_1.Elements stripe={stripePromise}>
        <PaymentForm tier={tier} price={price} onSuccess={onSuccess} onError={onError}/>
