...
exports.default = AlexAICrewDashboard;
const react_1 = __importStar(require("react"));
const n8n_data_service_1 = require("@/lib/n8n-data-service");
function AlexAICrewDashboard({ onDataUpdated }) {
    const [crewMembers, setCrewMembers] = (0, react_1.useState)([]);
    const [mcpKnowledge, setMcpKnowledge] = (0, react_1.useState)([]);
    const [n8nWorkflows, setN8nWorkflows] = (0, react_1.useState)([]);
    const [activeTab, setActiveTab] = (0, react_1.useState)('crew');
    ...
    const loadCrewData = async () => {
        try {
            ...
            const response = await n8n_data_service_1.n8nDataService.getCrewMembers();
    ...
    const loadMCPKnowledge = async () => {
        try {
            const response = await n8n_data_service_1.n8nDataService.getMCPKnowledge();
            if (response.success && response.data) {
    ...
    const startMCPScraping = async () => {
        try {
            const response = await n8n_data_service_1.n8nDataService.scrapeMCPKnowledge({
    ...
    const deployN8NWorkflow = async (workflowId) => {
        try {
            const response = await fetch('/api/n8n-workflows', {
                method: 'POST',
                headers: {
    ...
    const getCrewMemberColor = (crewId) => {
        switch (crewId) {
    ...
    return (<div className="bg-white rounded-lg shadow-md p-6">
      ...
      {/* Crew Members Tab */}
      {activeTab === 'crew' && (<div className="space-y-4">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-semibold text-gray-800">Alex AI Crew Members</h3>
            <button onClick={loadCrewData} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-sm">
              🔄 Refresh Crew Data
            </button>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {crewMembers.map(member => (<div key={member.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                ...
                <div className="space-y-2">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1 mt-1">
                      {member.expertise_areas.map(area => (<span key={area} className="px-2 py-1 bg-gray-100 text-gray-700 rounded text-xs">
                          {area}
                        </span>))}
                    </div>
                  </div>
      ...
      {activeTab === 'knowledge' && (<div className="space-y-4">
          <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={startMCPScraping} className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors text-sm">
              🔍 Start MCP Scraping
            </button>
          </div>
          
          <div className="space-y-3">
            {mcpKnowledge.length === 0 ? (<div className="text-center py-8 text-gray-500">
                <div className="text-4xl mb-2">🧠</div>
                <p>No MCP knowledge available yet. Start scraping to build the knowledge base!</p>
              </div>) : (mcpKnowledge.map(knowledge => (<div key={knowledge.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                  ...
                  <div className="flex flex-wrap gap-1 mb-2">
                    {knowledge.tags.map(tag => (<span key={tag} className="px-2 py-1 bg-gray-100 text-gray-700 rounded text-xs">
                        {tag}
                      </span>))}
                  </div>
      ...
      {activeTab === 'workflows' && (<div className="space-y-4">
          <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={loadN8NWorkflows} className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors text-sm">
              🔄 Refresh Workflows
            </button>
          </div>
          
          <div className="space-y-3">
            {n8nWorkflows.map(workflow => (<div key={workflow.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center space-x-3">
                  ...
                  <div className="flex items-center space-x-2">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${workflow.enabled ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
                      {workflow.enabled ? 'Enabled' : 'Disabled'}
                    </span>
                    <button onClick={() => deployN8NWorkflow(workflow.id)} className="px-3 py-1 bg-blue-600 text-white rounded text-xs hover:bg-blue-700 transition-colors">
                      Deploy
                    </button>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-4 text-sm">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1 mt-1">
                      {workflow.triggers.map(trigger => (<span key={trigger} className="px-2 py-1 bg-gray-100 text-gray-700 rounded text-xs">
                          {trigger}
                        </span>))}
                    </div>
                  </div>
      ...
      {/* Crew Memories Tab */}
      {activeTab === 'memories' && (<div className="space-y-4">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-semibold text-gray-800">Crew Memories & Insights</h3>
            <select value={selectedCrewMember} onChange={(e) => setSelectedCrewMember(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
              <option value="">All Crew Members</option>
              {crewMembers.map(member => (<option key={member.id} value={member.id}>
...
//# sourceMappingURL=AlexAICrewDashboard.js.map