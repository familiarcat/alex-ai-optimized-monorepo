...
exports.default = N8NUnificationDashboard;
const react_1 = __importStar(require("react"));
function N8NUnificationDashboard({ onUnificationComplete }) {
    const [crewMappings, setCrewMappings] = (0, react_1.useState)([]);
    const [unificationStatus, setUnificationStatus] = (0, react_1.useState)(null);
    const [activeTab, setActiveTab] = (0, react_1.useState)('mappings');
    const [isLoading, setIsLoading] = (0, react_1.useState)(false);
    const [selectedCrewMember, setSelectedCrewMember] = (0, react_1.useState)('');
    ...
    const getCrewMemberIcon = (crewId) => {
        switch (crewId) {
    ...
    const getFederationIcon = (federationMember) => {
        if (federationMember.includes('Geordi'))
            return '🔧';
        if (federationMember.includes('Data'))
            return '🤖';
        if (federationMember.includes('Troi'))
            return '👥';
        if (federationMember.includes('Crusher'))
            return '🌱';
        return '⭐';
    };
    ...
    return (<div className="bg-white rounded-lg shadow-md p-6">
      ...
      {/* Crew Mappings Tab */}
      {activeTab === 'mappings' && (<div className="space-y-4">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-semibold text-gray-800">Alex AI ↔ Federation Crew Mappings</h3>
            <button onClick={syncCrewMembers} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-sm">
              🔄 Sync Crew Members
            </button>
          </div>
          
          <div className="grid grid-cols-1 gap-4">
            {crewMappings.map(mapping => (<div key={mapping.alex_ai_crew_id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                ...
                <div className="mt-3">
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    {mapping.expertise_areas.map(area => (<span key={area} className="px-2 py-1 bg-gray-100 text-gray-700 rounded text-xs">
                        {area}
                      </span>))}
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-2 mt-1">
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {mapping.unified_capabilities.alex_ai_capabilities.map(cap => (<span key={cap} className="px-1 py-0.5 bg-blue-100 text-blue-700 rounded text-xs">
                            {cap}
                          </span>))}
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {mapping.unified_capabilities.federation_capabilities.map(cap => (<span key={cap} className="px-1 py-0.5 bg-green-100 text-green-700 rounded text-xs">
                            {cap}
                          </span>))}
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {mapping.unified_capabilities.unified_capabilities.map(cap => (<span key={cap} className="px-1 py-0.5 bg-purple-100 text-purple-700 rounded text-xs">
                            {cap}
                          </span>))}
                      </div>
                    </div>
      ...
      {activeTab === 'status' && (<div className="space-y-4">
          <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={loadUnificationData} className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors text-sm">
              🔄 Refresh Status
            </button>
          </div>
          
          {unificationStatus ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      {activeTab === 'workflows' && (<div className="space-y-4">
          <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={unifyWorkflows} className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors text-sm">
              ⚡ Unify Workflows
            </button>
          </div>
      ...
      {activeTab === 'analysis' && (<div className="space-y-4">
          <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={performCrossCrewAnalysis} className="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors text-sm">
              🔍 Perform Analysis
            </button>
          </div>
          
          {analysisResults.length === 0 ? (<div className="text-center py-8 text-gray-500">
              <div className="text-4xl mb-2">🔍</div>
              ...
              {analysisResults.map((result, index) => (<div key={index} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {result.status === 'error' ? (<span className="px-2 py-1 bg-red-100 text-red-800 rounded text-xs font-medium">
                        Error
                      </span>) : (<span className="px-2 py-1 bg-green-100 text-green-800 rounded text-xs font-medium">
                        Success
                      </span>)}
                  </div>

                  {result.status !== 'error' && (<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <span className="text-sm font-medium text-gray-700">Alex AI Analysis:</span>
                        <div className="mt-1">
                          {result.alex_ai_analysis.insights.map((insight, i) => (<p key={i} className="text-sm text-gray-600">• {insight}</p>))}
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="mt-1">
                          {result.federation_analysis.insights.map((insight, i) => (<p key={i} className="text-sm text-gray-600">• {insight}</p>))}
                        </div>
                      </div>
                    </div>)}

                  {result.status === 'error' && (<div className="text-red-600 text-sm">
                      Error: {result.error}
                    </div>)}
                </div>))}
            </div>)}
...
//# sourceMappingURL=N8NUnificationDashboard.js.map