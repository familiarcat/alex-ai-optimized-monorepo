...
exports.default = StealthScrapingDashboard;
const react_1 = require("react");
const axios_1 = __importDefault(require("axios"));
function StealthScrapingDashboard() {
    const [scrapingJobs, setScrapingJobs] = (0, react_1.useState)([]);
    ...
    const fetchScrapingJobs = async () => {
        try {
            const response = await axios_1.default.get('/api/stealth-job-scraping');
    ...
    const startStealthScraping = async () => {
        setLoading(true);
        try {
            const response = await axios_1.default.post('/api/stealth-job-scraping', formData);
    ...
    return (<div className="bg-white rounded-lg shadow-lg p-6">
      ...
      <div className="bg-gray-50 rounded-lg p-4 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            ...
            <select value={formData.source} onChange={(e) => setFormData({ ...formData, source: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
          ...
          <div>
            ...
            <input type="text" value={formData.searchTerm} onChange={(e) => setFormData({ ...formData, searchTerm: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="AI Engineer"/>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Location</label>
            <input type="text" value={formData.location} onChange={(e) => setFormData({ ...formData, location: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="St. Louis, MO"/>
          </div>
          <div className="flex items-end">
            <button onClick={startStealthScraping} disabled={loading} className="w-full bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed">
      ...
      <div>
        ...
        {scrapingJobs.length === 0 ? (<div className="text-center py-8 text-gray-500">
            ...
            {scrapingJobs.map((job) => (<div key={job.id} className="bg-white border border-gray-200 rounded-lg p-4">
...
//# sourceMappingURL=StealthScrapingDashboard.js.map