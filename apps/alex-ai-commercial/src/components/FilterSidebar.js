...
function FilterSidebar({ filters, onFiltersChange, skillPriorities, onSkillPrioritiesChange }) {
    const [isExpanded, setIsExpanded] = (0, react_1.useState)(true);
    const [skillsExpanded, setSkillsExpanded] = (0, react_1.useState)(false);
    ...
    const handleSkillPriorityChange = (skill, priority) => {
    ...
    const getWorkLifeBalanceLabel = (value) => {
        const labels = ['Low', 'Below Average', 'Average', 'High', 'Very High'];
    ...
    return (<div className="bg-white rounded-lg shadow-sm border p-6">
      ...
      {isExpanded && (<framer_motion_1.motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="space-y-6">
          {/* Location Filter */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Location Preference
            </label>
            <select value={filters.location} onChange={(e) => onFiltersChange({ ...filters, location: e.target.value })} className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
          ...
          <div>
            ...
            <input type="range" min="1" max="5" value={filters.workLifeBalance} onChange={(e) => onFiltersChange({ ...filters, workLifeBalance: parseInt(e.target.value) })} className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer slider"/>
          ...
          <div>
            ...
            <input type="range" min="50" max="95" value={filters.alexAIScore} onChange={(e) => onFiltersChange({ ...filters, alexAIScore: parseInt(e.target.value) })} className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer slider"/>
            <div className="flex justify-between text-xs text-gray-500 mt-1">
              <span>50</span>
              <span>95</span>
            </div>
          ...
          <div>
            ...
            <select value={filters.salaryRange} onChange={(e) => onFiltersChange({ ...filters, salaryRange: e.target.value })} className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
          ...
          <div>
            ...
            <div className="space-y-2">
              {companyTypeOptions.map((option) => (<label key={option.value} className="flex items-center">
                  <input type="checkbox" checked={filters.companyTypes.includes(option.value)} onChange={(e) => handleCompanyTypeChange(option.value, e.target.checked)} className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"/>
          ...
          <div>
            <button onClick={() => setSkillsExpanded(!skillsExpanded)} className="flex items-center justify-between w-full text-left text-sm font-medium text-gray-700 mb-2 hover:text-gray-900">
            ...
            {skillsExpanded && (<framer_motion_1.motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="space-y-4">
                {Object.entries(skillCategories).map(([category, skills]) => (<div key={category} className="space-y-2">
                    <h4 className="text-xs font-medium text-gray-600 uppercase tracking-wide">
                      {category}
                    </h4>
                    <div className="space-y-2">
                      {skills.map(skill => (<div key={skill} className="space-y-1">
                          ...
                          <input type="range" min="1" max="5" value={getSkillPriority(skill)} onChange={(e) => handleSkillPriorityChange(skill, parseInt(e.target.value))} className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer slider"/>
                        </div>))}
                    </div>
                  </div>))}
                ...
                <div className="mt-4 p-3 bg-gray-50 rounded-lg">
                  ...
                  <div className="space-y-1">
                    ...
                    .sort(([_, a], [__, b]) => b - a)
                    .slice(0, 5)
                    .map(([skill, priority]) => (<div key={skill} className="flex items-center justify-between text-xs">
