...
function PricingTiers() {
    const [selectedTier, setSelectedTier] = (0, react_1.useState)('premium');
    return (<div className="bg-white py-24 sm:py-32">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="mt-16 grid grid-cols-1 gap-8 lg:grid-cols-3">
          {pricingTiers.map((tier) => (<div key={tier.id} className={`relative rounded-2xl border p-8 ${tier.popular
              ...
              {tier.popular && (<div className="absolute -top-4 left-1/2 -translate-x-1/2">
                  <span className="inline-flex items-center rounded-full bg-indigo-600 px-4 py-1 text-sm font-medium text-white">
                    <outline_1.StarIcon className="h-4 w-4 mr-1"/>
                    Most Popular
                  </span>
                </div>)}
              ...
              <ul className="mt-8 space-y-3">
                {tier.features.map((feature) => (<li key={feature} className="flex items-start">
                    <outline_1.CheckIcon className="h-5 w-5 text-indigo-600 mt-0.5 mr-3 flex-shrink-0"/>
              ...
              <div className="mt-4 p-4 bg-purple-50 rounded-lg">
                ...
                <ul className="mt-2 text-xs text-purple-700">
                  {tier.crewOversight.map((crew, index) => (<li key={index}>• {crew}</li>))}
                </ul>
              </div>

              <button className={`mt-8 w-full rounded-md px-3 py-2 text-center text-sm font-semibold ${tier.popular
                ? 'bg-indigo-600 text-white hover:bg-indigo-500'
                : 'bg-gray-100 text-gray-900 hover:bg-gray-200'}`} onClick={() => setSelectedTier(tier.id)}>
