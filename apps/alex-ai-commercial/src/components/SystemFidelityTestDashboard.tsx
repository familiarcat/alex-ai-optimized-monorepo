...
interface SystemFidelitySummary {
  total_tests: number
...
interface SystemAnalysis {
  ...
  mcp_tool_effectiveness: {
    total_tools_required: number
    total_tools_utilized: number
...
export default function SystemFidelityTestDashboard({ onTestComplete }: SystemFidelityTestDashboardProps) {
  const [testResults, setTestResults] = useState<FidelityTestResult[]>([])
  const [testSummary, setTestSummary] = useState<SystemFidelitySummary | null>(null)
  const [systemAnalysis, setSystemAnalysis] = useState<SystemAnalysis | null>(null)
  const [isRunning, setIsRunning] = useState(false)
  const [currentTest, setCurrentTest] = useState<string>('')
  const [availableTests, setAvailableTests] = useState<string[]>([])
  const [recommendations, setRecommendations] = useState<string[]>([])
  const [activeTab, setActiveTab] = useState<'overview' | 'results' | 'analysis' | 'recommendations'>('overview')
  ...
  const runSingleFidelityTest = async (testType: string) => {
    try {
      setIsRunning(true)
      ...
      if (result.success) {
        ...
        const newResult: FidelityTestResult = {
  ...
  const getFidelityScoreColor = (score: number) => {
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            {availableTests.map(testType => (
              <button
                key={testType}
                onClick={() => runSingleFidelityTest(testType)}
                disabled={isRunning}
