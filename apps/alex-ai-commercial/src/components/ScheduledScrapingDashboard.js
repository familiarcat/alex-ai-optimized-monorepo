...
exports.default = ScheduledScrapingDashboard;
const react_1 = __importStar(require("react"));
const useUserCentricJobScrapingPolling_1 = require("@/hooks/useUserCentricJobScrapingPolling");
const user_analytics_simple_1 = require("@/lib/user-analytics-simple");
function ScheduledScrapingDashboard({ onJobsUpdated }) {
    const [isInitializing, setIsInitializing] = (0, react_1.useState)(false);
    const [isRunningAll, setIsRunningAll] = (0, react_1.useState)(false);
    const [showCreateForm, setShowCreateForm] = (0, react_1.useState)(false);
    const [newConfig, setNewConfig] = (0, react_1.useState)({
        name: '',
        source: 'linkedin',
    ...
    const { data: scheduledData, loading, error, state: pollingState, forceRefresh } = (0, useUserCentricJobScrapingPolling_1.useUserCentricScheduledScrapingPolling)();
    ...
    (0, react_1.useEffect)(() => {
        user_analytics_simple_1.userAnalytics.trackPageView('scheduled_scraping_dashboard');
    }, []);
    ...
    const runConfigNow = async (configId) => {
        try {
            const response = await fetch('/api/scheduled-scraping', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    return (<div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex space-x-2">
            {configs.length === 0 && (<button onClick={initializeDefaultConfigs} disabled={isInitializing} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50">
                {isInitializing ? 'Initializing...' : 'Initialize Defaults'}
              </button>)}
            <button onClick={runAllConfigs} disabled={isRunningAll || configs.filter(c => c.enabled).length === 0} className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50">
              {isRunningAll ? 'Running...' : 'Run All Now'}
            </button>
            <button onClick={() => setShowCreateForm(!showCreateForm)} className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700">
        ...
        {/* Status Overview */}
        {status && (<div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        {/* Create Form */}
        {showCreateForm && (<div className="bg-gray-50 p-4 rounded-lg mb-6">
            <h3 className="text-lg font-semibold mb-4">Create New Scheduled Configuration</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Name</label>
                <input type="text" value={newConfig.name} onChange={(e) => setNewConfig({ ...newConfig, name: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Configuration name"/>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Source</label>
                <select value={newConfig.source} onChange={(e) => setNewConfig({ ...newConfig, source: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
              ...
              <div>
                ...
                <input type="text" value={newConfig.search_term} onChange={(e) => setNewConfig({ ...newConfig, search_term: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Job search term"/>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Location</label>
                <select value={newConfig.location} onChange={(e) => setNewConfig({ ...newConfig, location: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                  {locations.map(location => (<option key={location} value={location}>{location}</option>))}
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Max Results</label>
                <input type="number" value={newConfig.max_results} onChange={(e) => setNewConfig({ ...newConfig, max_results: parseInt(e.target.value) })} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" min="1" max="100"/>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Schedule</label>
                <select value={newConfig.schedule} onChange={(e) => setNewConfig({ ...newConfig, schedule: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                  {schedules.map(schedule => (<option key={schedule.value} value={schedule.value}>
            ...
            <div className="flex justify-end space-x-2 mt-4">
              ...
              <button onClick={createConfig} disabled={!newConfig.name} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50">
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        {configs.length === 0 ? (<div className="text-center py-8 text-gray-500">
            <div className="text-4xl mb-2">⏰</div>
            <p>No scheduled configurations found.</p>
            <p className="text-sm">Click "Initialize Defaults" to get started.</p>
          </div>) : (<div className="space-y-4">
            {configs.map(config => (<div key={config.id} className="border border-gray-200 rounded-lg p-4">
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    <div className="flex items-center space-x-2 mb-2">
                      <h4 className="font-semibold text-gray-900">{config.name}</h4>
                      <span className={`px-2 py-1 rounded-full text-xs ${config.enabled ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
                    ...
                    {config.next_run && (<div className="mt-2 text-sm">
                  ...
                  <div className="flex space-x-2 ml-4">
                    <button onClick={() => toggleConfig(config.id)} className={`px-3 py-1 rounded text-sm ${config.enabled
                    ? 'bg-yellow-100 text-yellow-800 hover:bg-yellow-200'
                    : 'bg-green-100 text-green-800 hover:bg-green-200'}`}>
                      {config.enabled ? 'Disable' : 'Enable'}
                    </button>
                    <button onClick={() => runConfigNow(config.id)} className="px-3 py-1 bg-blue-100 text-blue-800 rounded text-sm hover:bg-blue-200">
                      Run Now
                    </button>
                    <button onClick={() => deleteConfig(config.id)} className="px-3 py-1 bg-red-100 text-red-800 rounded text-sm hover:bg-red-200">
                      Delete
                    </button>
                  </div>
                </div>
        ...
        {recentJobs.length === 0 ? (<div className="text-center py-8 text-gray-500">
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {recentJobs.slice(0, 10).map(job => (<tr key={job.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      <span className={`px-2 py-1 text-xs rounded-full ${job.triggered_by === 'scheduled' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'}`}>
                        {job.triggered_by}
                      </span>
                    </td>
...
//# sourceMappingURL=ScheduledScrapingDashboard.js.map