...
exports.default = ApplicationTracker;
const react_1 = require("react");
const framer_motion_1 = require("framer-motion");
const outline_1 = require("@heroicons/react/24/outline");
function ApplicationTracker({ applications, jobOpportunities }) {
    ...
    const getStatusIcon = (status) => {
        switch (status) {
            case 'applied': return outline_1.ClockIcon;
            case 'interview_scheduled': return outline_1.CalendarIcon;
            case 'interviewed': return outline_1.EyeIcon;
            case 'offer_received': return outline_1.CheckCircleIcon;
            case 'rejected': return outline_1.XCircleIcon;
            case 'withdrawn': return outline_1.ExclamationTriangleIcon;
            default: return outline_1.ClockIcon;
        }
    };
    ...
    return (<div className="bg-white rounded-lg shadow-sm border p-6">
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        {Object.entries(statusCounts).map(([status, count]) => (<button key={status} onClick={() => setSelectedStatus(status)} className={`px-3 py-1 rounded-full text-sm font-medium transition-colors ${selectedStatus === status
      ...
      <div className="space-y-4">
        {filteredApplications.length === 0 ? (<div className="text-center py-8">
            <p className="text-gray-500">No applications found for the selected status.</p>
          </div>) : (filteredApplications.map((application, index) => {
            const job = getJobDetails(application.job_id);
            const StatusIcon = getStatusIcon(application.status);
            const daysSince = getDaysSinceApplication(application.application_date);
            return (<framer_motion_1.motion.div key={application.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-gray-600">
                      ...
                      {application.response_date && (<div>
                          <span className="font-medium">Response:</span> {formatDate(application.response_date)}
                        </div>)}
                      
                      {application.interview_date && (<div>
                          <span className="font-medium">Interview:</span> {formatDate(application.interview_date)}
                        </div>)}
                    </div>

                    {application.notes && (<div className="mt-3 p-3 bg-gray-50 rounded-lg">
                        <p className="text-sm text-gray-700">{application.notes}</p>
                      </div>)}

                    {job && (<div className="mt-3 flex items-center space-x-4 text-sm">
                  ...
                  <div className="flex flex-col space-y-2 ml-4">
                    {job?.application_url && (<a href={job.application_url} target="_blank" rel="noopener noreferrer" className="text-indigo-600 hover:text-indigo-800 text-sm font-medium">
                        View Job →
                      </a>)}
                    
                    {application.status === 'applied' && daysSince > 7 && (<button className="text-yellow-600 hover:text-yellow-800 text-sm font-medium">
                        Follow Up
                      </button>)}
                  </div>
                </div>
      ...
      {/* Summary Stats */}
      {applications.length > 0 && (<div className="mt-6 pt-4 border-t border-gray-200">
...
//# sourceMappingURL=ApplicationTracker.js.map