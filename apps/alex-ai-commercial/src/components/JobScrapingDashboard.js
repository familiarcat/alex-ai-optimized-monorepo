...
exports.default = JobScrapingDashboard;
const react_1 = __importStar(require("react"));
const useUserCentricJobScrapingPolling_1 = require("@/hooks/useUserCentricJobScrapingPolling");
const useJobScrapingEvents_1 = require("@/hooks/useJobScrapingEvents");
const user_analytics_simple_1 = require("@/lib/user-analytics-simple");
function JobScrapingDashboard({ onJobsUpdated, onDataSourceUpdate }) {
    const [isScraping, setIsScraping] = (0, react_1.useState)(false);
    const [selectedSource, setSelectedSource] = (0, react_1.useState)('linkedin');
    const [searchTerm, setSearchTerm] = (0, react_1.useState)('AI Engineer');
    const [location, setLocation] = (0, react_1.useState)('St. Louis, MO');
    const [maxResults, setMaxResults] = (0, react_1.useState)(10);
    // Use user-centric polling with Server-Sent Events fallback
    const { data: pollingJobs, loading: pollingLoading, state: pollingState, forceRefresh } = (0, useUserCentricJobScrapingPolling_1.useUserCentricJobScrapingPolling)();
    const { jobs: eventJobs, isConnected: eventsConnected, hasData: eventsHasData } = (0, useJobScrapingEvents_1.useLatestJobStatus)();
    ...
    (0, react_1.useEffect)(() => {
        user_analytics_simple_1.userAnalytics.trackPageView('job_scraping_dashboard');
    }, []);
    const startScraping = async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                await user_analytics_simple_1.userAnalytics.trackScrapingTrigger(selectedSource);
    ...
    return (<div className="bg-white rounded-lg shadow-md p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="flex items-center space-x-2">
            <div className={`w-2 h-2 rounded-full ${eventsConnected ? 'bg-green-500' :
            pollingState?.isUserActive ? 'bg-blue-500' : 'bg-yellow-500'}`}></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div>
          ...
          <select value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
            {searchTerms.map(term => (<option key={term} value={term}>{term}</option>))}
          </select>
        </div>
        ...
        <div>
          ...
          <select value={location} onChange={(e) => setLocation(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
        ...
        <div>
          ...
          <input type="number" value={maxResults} onChange={(e) => setMaxResults(parseInt(e.target.value))} min="1" max="50" className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"/>
        </div>
      </div>

      {/* Start Scraping Button */}
      <div className="mb-6">
        <button onClick={startScraping} disabled={isScraping} className={`px-6 py-3 rounded-lg font-medium transition-colors ${isScraping
      ...
      <div className="space-y-4">
        ...
        {scrapingJobs.length === 0 ? (<div className="text-center py-8 text-gray-500">
            <div className="text-4xl mb-2">🔍</div>
            <p>No scraping jobs yet. Start your first job scraping session!</p>
          </div>) : (<div className="space-y-3">
            {scrapingJobs.map((job) => (<div key={job.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  ...
                  {job.completed_at && (<div>
                      <span className="text-gray-500">Completed:</span>
                      <span className="ml-1 font-medium">{formatDate(job.completed_at)}</span>
                    </div>)}
                </div>

                {job.status_message && (<div className="mt-2 text-sm text-gray-600">
...
//# sourceMappingURL=JobScrapingDashboard.js.map