...
exports.default = EndToEndTestDashboard;
const react_1 = __importStar(require("react"));
function EndToEndTestDashboard({ onTestComplete }) {
    ...
    return (<div className="bg-white rounded-lg shadow-md p-6">
      ...
      {/* Test Summary */}
      {testSummary && (<div className="mb-6 p-4 bg-gray-50 rounded-lg">
      ...
      <div className="mb-6 flex flex-wrap gap-3">
        <button onClick={runAllTests} disabled={isRunning} className="px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-2">
          <span>🚀</span>
          <span>{isRunning ? 'Running All Tests...' : 'Run All Tests'}</span>
        </button>
        
        {availableTests.map(testType => (<button key={testType} onClick={() => runSingleTest(testType)} disabled={isRunning} className="px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-2">
      ...
      {/* Current Test Status */}
      {isRunning && currentTest && (<div className="mb-6 p-4 bg-blue-50 rounded-lg">
      ...
      {/* Test Results */}
      {testResults.length > 0 && (<div className="mb-6">
          <h3 className="text-lg font-semibold text-gray-800 mb-3">Test Results</h3>
          <div className="space-y-3">
            {testResults.map((result, index) => (<div key={index} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                ...
                {result.status === 'success' && result.result && (<div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div>
      ...
      {recommendations.length > 0 && (<div className="mb-6">
          ...
          <div className="space-y-2">
            {recommendations.map((recommendation, index) => (<div key={index} className="flex items-start space-x-2 p-3 bg-gray-50 rounded-lg">
      ...
      {/* Test Instructions */}
      {testResults.length === 0 && !isRunning && (<div className="text-center py-8 text-gray-500">
      ...
      {isRunning && (<div className="flex items-center justify-center py-8">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600"></div>
          <span className="ml-2 text-gray-600">Running tests...</span>
        </div>)}
    </div>);
}
//# sourceMappingURL=EndToEndTestDashboard.js.map