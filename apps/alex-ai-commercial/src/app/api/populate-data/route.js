...
async function POST() {
    try {
        ...
        await supabase_1.supabase.from('job_opportunities').delete().neq('id', 0);
        console.log('🧹 Clearing existing contacts...');
        await supabase_1.supabase.from('contacts').delete().neq('id', 0);
        ...
        const { data: jobsData, error: jobsError } = await supabase_1.supabase
