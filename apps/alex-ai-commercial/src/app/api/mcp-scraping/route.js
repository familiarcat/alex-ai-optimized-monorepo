...
async function GET(request) {
    try {
        ...
        if (category) {
            ...
            return server_1.NextResponse.json(knowledge);
        }
...
async function startMCPScrapingJob(source, category, maxResults) {
...
async function performMCPScraping(jobId, source, category, maxResults) {
    try {
...
async function scrapeSpecificSource(source, maxResults) {
    const sourceConfig = MCP_SOURCES[source];
    if (!sourceConfig)
        return [];
...
async function scrapeGitHubAPI(url, maxResults) {
...
async function scrapeDocumentation(url, maxResults) {
...
async function categorizeDataForCrew(data, category) {
...
function determineCrewRelevance(item, category) {
...
function calculateRelevanceScore(tags, content, title, keywords) {
    let score = 0;
    // Check tags
    tags.forEach(tag => {
...
function generateInsights(item, type) {
    const insights = [];
    switch (type) {
        case 'technical':
...
function generateRecommendations(item, type) {
...
async function storeMCPKnowledge(jobId, data) {
...
async function getMCPScrapingJobStatus(jobId) {
