...
async function GET(request) {
    try {
        ...
        if (workflow) {
            ...
            return server_1.NextResponse.json(workflowStatus);
        }
        if (status) {
            // Get workflows by status
            const workflows = await getWorkflowsByStatus(status);
            return server_1.NextResponse.json(workflows);
        }
        // Get all workflows
        const allWorkflows = await listWorkflows();
        return server_1.NextResponse.json(allWorkflows);
    }
    catch (error) {
...
async function deployWorkflow(workflow, config) {
...
async function executeWorkflow(workflow, config) {
...
async function getWorkflowsByStatus(status) {
    const allWorkflows = await listWorkflows();
