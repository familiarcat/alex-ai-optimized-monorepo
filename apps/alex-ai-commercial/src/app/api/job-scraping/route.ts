...
export async function POST(request: Request) {
  try {
    ...
    // Validate source
    if (!JOB_SOURCES[source as keyof typeof JOB_SOURCES]) {
...
async function scrapeLinkedInJobs(searchTerm: string, location: string, maxResults: number) {
...
async function scrapeIndeedJobs(searchTerm: string, location: string, maxResults: number) {
...
async function scrapeGlassdoorJobs(searchTerm: string, location: string, maxResults: number) {
...
async function scrapeCompanyCareerPages(searchTerm: string, location: string, maxResults: number) {
...
async function analyzeJobsWithAlexAI(jobs: any[]) {
...
async function storeScrapedJobs(jobId: string, jobs: any[]) {
...
async function updateScrapingJobStatus(jobId: string, status: string, message?: string) {
  // For now, just log the status update
  console.log(`Job ${jobId} status updated to: ${status} - ${message}`)
}

async function getScrapingJobStatus(jobId: string) {
...
async function getScrapingJobsByStatus(status: string) {
