...
async function POST(request) {
    try {
        ...
        if (test_type && SYSTEM_FIDELITY_TESTS[test_type]) {
...
async function runAllFidelityTests(problemCustomization) {
...
async function runSingleFidelityTest(testType, problemCustomization) {
    const testScenario = SYSTEM_FIDELITY_TESTS[testType];
...
async function executeFidelityTest(testKey, testScenario, problemCustomization) {
    const startTime = Date.now();
...
function analyzeMCPToolUtilization(requiredTools, responseData) {
    if (!responseData || !responseData.mcp_analysis)
        return 0;
    let utilizedTools = 0;
...
function analyzeCrewCollaboration(collaboration, responseData) {
    if (!responseData || !responseData.crew_analysis)
        return 'failed';
    ...
    if (hasAlexAI && hasFederation && hasCollaboration)
        return 'excellent';
    if (hasAlexAI && hasFederation)
        return 'good';
    if (hasAlexAI || hasFederation)
        return 'partial';
    return 'failed';
}
function analyzeProblemSolvingEffectiveness(problem, responseData) {
    if (!responseData || !responseData.solution_analysis)
        return 0;
    let criteriaMet = 0;
    problem.success_criteria.forEach((criterion) => {
        if (responseData.solution_analysis.recommendations?.some((rec) => rec.toLowerCase().includes(criterion.toLowerCase().substring(0, 20)))) {
            criteriaMet++;
        }
    });
    return Math.round((criteriaMet / problem.success_criteria.length) * 100);
}
function calculateSystemFidelityScore(testResults) {
    const weights = {
        success_rate: 0.3,
    ...
    const fidelityScore = ((testResults.filter(r => r.status === 'success').length / testResults.length) * 100 * weights.success_rate +
...
function generateSystemAnalysis(testResults) {
    const successfulTests = testResults.filter(r => r.status === 'success');
    return {
...
function generateFidelityRecommendations(testResults) {
...
function analyzeSingleTestFidelity(result, testScenario) {
    return {
...
function generateSingleTestFidelityRecommendations(result, testScenario) {
    const recommendations = [];
    if (result.success) {
...
function generateFidelityFailureRecommendations(testType, error) {
    const recommendations = [];
