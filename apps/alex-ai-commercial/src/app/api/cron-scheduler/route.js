...
async function triggerScheduledJob(configId) {
...
async function getSchedulerStatus() {
    try {
        ...
        const { data: configs, error: configsError } = await supabase_1.supabase
            .from('scheduled_scraping_configs')
            .select('id, enabled, schedule, last_run, next_run');
        if (configsError)
            throw configsError;
        // Get recent job counts
        const { data: recentJobs, error: jobsError } = await supabase_1.supabase
            .from('scraping_jobs')
...
async function logScheduleExecution(configId, jobId, action, details) {
    try {
        await supabase_1.supabase
