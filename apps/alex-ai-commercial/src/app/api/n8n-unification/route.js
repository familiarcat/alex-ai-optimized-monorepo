...
async function POST(request) {
    try {
        ...
        switch (action) {
            ...
            default:
                return server_1.NextResponse.json({ success: false, error: `Invalid action: ${action}` }, { status: 400 });
        }
    }
    catch (error) {
...
async function GET(request) {
    try {
        ...
        if (crew_member) {
            ...
            return server_1.NextResponse.json(mapping);
        }
        if (status) {
        ...
        return server_1.NextResponse.json(allMappings);
    }
    catch (error) {
...
async function federationConsultation(crewMember, data) {
    console.log(`🤝 Federation consultation for ${crewMember}...`);
    const mapping = N8N_UNIFICATION_CONFIG.federation_crew_mapping[crewMember];
    if (!mapping) {
...
async function alexAIToFederation(crewMember, data) {
...
async function federationToAlexAI(crewMember, data) {
...
async function crossCrewAnalysis(data) {
...
// Helper functions
function getCrewMemberMapping(crewMember) {
    return N8N_UNIFICATION_CONFIG.federation_crew_mapping[crewMember] || null;
}
...
async function testFederationCrewMember(webhookPath) {
...
function generateUnifiedCapabilities(alexCrewId, federationMapping) {
    return {
...
async function deployUnifiedWorkflow(config) {
...
async function sendToFederationCrew(webhookPath, data) {
...
async function processFederationDataForAlexAI(data, crewMember) {
...
function generateUnifiedInsights(alexAIData, federationResponse) {
    return {
...
function generateAlexAIInsights(processedData, crewMember) {
    return {
        crew_member: crewMember,
        insights: [
...
async function getAlexAICrewAnalysis(crewMember, data) {
...
async function getFederationCrewAnalysis(webhookPath, data) {
...
function generateUnifiedAnalysis(alexAIAnalysis, federationAnalysis) {
    return {
...
function generateCrossCrewInsights(alexAIAnalysis, federationAnalysis) {
    return [
...
function generateFederationInsights(data) {
    return [
...
function generateFederationRecommendations(data) {
    return [
        'Continue cross-crew collaboration',
