...
async function POST(request) {
    try {
        ...
        if (test_type && TEST_SCENARIOS[test_type]) {
...
async function runSingleTest(testType) {
    const testScenario = TEST_SCENARIOS[testType];
...
async function executeTest(testKey, testScenario) {
    const startTime = Date.now();
    try {
...
function generateSingleTestRecommendations(result) {
    const recommendations = [];
    if (result.success) {
...
function generateFailureRecommendations(testType, error) {
    const recommendations = [];
