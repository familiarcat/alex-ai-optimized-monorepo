...
const DEFAULT_SCRAPING_CONFIGS: Omit<ScheduledScrapingConfig, 'id' | 'created_at' | 'updated_at'>[] = [
  {
...
export async function GET(request: Request) {
  try {
    ...
    if (action === 'status') {
      ...
      const status = {
        ...
        lastRun: configs.reduce((latest, config) => {
          if (!config.lastRun) return latest
        ...
        nextRun: configs.reduce((earliest, config) => {
          if (!config.nextRun) return earliest
    ...
    return NextResponse.json({ success: true, jobs: recentJobs })
...
// Helper functions
async function getScheduledConfigs(): Promise<ScheduledScrapingConfig[]> {
  try {
...
async function createScheduledConfig(config: Omit<ScheduledScrapingConfig, 'id' | 'created_at' | 'updated_at'>) {
  const { data, error } = await supabase
    .from('scheduled_scraping_configs')
    .insert([{
      ...config,
...
async function updateScheduledConfig(configId: string, updates: Partial<ScheduledScrapingConfig>) {
  const { data, error } = await supabase
    .from('scheduled_scraping_configs')
    .update({
      ...updates,
...
async function toggleScheduledConfig(configId: string) {
  const { data, error } = await supabase
    .from('scheduled_scraping_configs')
    .update({
      enabled: supabase.raw('NOT enabled'),
      updated_at: new Date().toISOString()
    })
    .eq('id', configId)
    .select()
...
async function runScheduledScraping(configId: string) {
...
async function getScheduledConfig(configId: string): Promise<ScheduledScrapingConfig | null> {
  const { data, error } = await supabase
    .from('scheduled_scraping_configs')
    .select('*')
    .eq('id', configId)
...
async function deleteScheduledConfig(configId: string) {
  const { data, error } = await supabase
    .from('scheduled_scraping_configs')
    .delete()
    .eq('id', configId)
