...
import { n8nDataService, JobOpportunity, Contact, Application } from '@/lib/n8n-data-service'
...
import { alexAIJobOpportunities, alexAIContacts, alexAIMemorySystem } from '@/lib/mock-alex-ai-data'
import JobCard from '@/components/JobCard'
import ResumeUpload from '@/components/ResumeUpload'
import FilterSidebar from '@/components/FilterSidebar'
import StatsDashboard from '@/components/StatsDashboard'
import ApplicationTracker from '@/components/ApplicationTracker'
import JobScrapingDashboard from '@/components/JobScrapingDashboard'
import StealthScrapingDashboard from '@/components/StealthScrapingDashboard'
import ScheduledScrapingDashboard from '@/components/ScheduledScrapingDashboard'
import AlexAICrewDashboard from '@/components/AlexAICrewDashboard'
import N8NUnificationDashboard from '@/components/N8NUnificationDashboard'
import EndToEndTestDashboard from '@/components/EndToEndTestDashboard'
import SystemFidelityTestDashboard from '@/components/SystemFidelityTestDashboard'
import DataSourceIndicator from '@/components/DataSourceIndicator'
import DataSourceIndicatorTest from '@/components/DataSourceIndicatorTest'
import AutoStealthScrapingDashboard from '@/components/AutoStealthScrapingDashboard'
import { useDataSourceTracker } from '@/hooks/useDataSourceTracker'
import { motion } from 'framer-motion'
import { N8NHealthStatus } from '@/lib/n8n-health-manager'

interface ClientPageProps {
  initialHealthStatus: N8NHealthStatus
}

export default function ClientPage({ initialHealthStatus }: ClientPageProps) {
  ...
  const [showStealthScrapingDashboard, setShowStealthScrapingDashboard] = useState(false)
  const [showScheduledScrapingDashboard, setShowScheduledScrapingDashboard] = useState(false)
  const [showCrewDashboard, setShowCrewDashboard] = useState(false)
  const [showUnificationDashboard, setShowUnificationDashboard] = useState(false)
  const [showTestDashboard, setShowTestDashboard] = useState(false)
  const [showFidelityTestDashboard, setShowFidelityTestDashboard] = useState(false)
  const [showDataSourceTest, setShowDataSourceTest] = useState(false)
  const [showAutoStealthDashboard, setShowAutoStealthDashboard] = useState(false)

  // Data source tracking
  const { dataSource, updateDataSource, fetchScrapingJobs } = useDataSourceTracker()
  ...
  const loadData = async () => {
    try {
      ...
      const contactsResponse = await n8nDataService.getContacts()
      ...
      const applicationsResponse = await n8nDataService.getApplications()
  ...
  const loadDefaultResume = async () => {
    try {
      ...
      if (jobOpportunities.length > 0) {
        ...
        try {
          const jobMatching = await alexAI.matchJobsToResume(basicAnalysis, jobOpportunities)
  ...
  const calculateSkillsMatchScore = useCallback((job: JobOpportunity): number => {
  ...
  useEffect(() => {
    ...
    if (process.env.NODE_ENV === 'production') {
      n8nSyncService.startSync(5) // Sync every 5 minutes
    }
    
    // Cleanup on unmount
    return () => {
      n8nSyncService.stopSync()
    }
  }, [])
  ...
  const handleResumeAnalysis = async (resumeFile: File) => {
    try {
      const analysis = await alexAI.analyzeResume(resumeFile)
      setResumeAnalysis(analysis)
  ...
  const handleJobApplication = async (job: JobOpportunity) => {
    try {
      // Create application record through N8N
      const applicationResponse = await n8nDataService.createApplication({
        job_id: job.id,
        status: 'applied',
      ...
      await alexAI.trackApplication(
        application.id,
        'application_submitted',
        { job_id: job.id, company: job.company, position: job.position }
      )
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
          ...
          <div className="flex justify-between items-center py-6">
            <div>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                ...
                <button
                  onClick={() => setShowFidelityTestDashboard(!showFidelityTestDashboard)}
                  className="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors text-sm"
                >
                  {showFidelityTestDashboard ? '🎯 Hide' : '🎯 Show'} System Fidelity Test Dashboard
                </button>
                <button
                  onClick={() => setShowDataSourceTest(!showDataSourceTest)}
                ...
                <button
                  onClick={() => setShowAutoStealthDashboard(!showAutoStealthDashboard)}
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-1">
            <FilterSidebar
              ...
              onSkillPrioritiesChange={setSkillPriorities}
            />
        ...
        {/* Dashboards */}
        {showScrapingDashboard && (
          <div className="mt-12">
            <JobScrapingDashboard />
          </div>
        )}

        {showStealthScrapingDashboard && (
          <div className="mt-12">
            <StealthScrapingDashboard />
          </div>
        )}

        {showScheduledScrapingDashboard && (
          <div className="mt-12">
        ...
        {showUnificationDashboard && (
          <div className="mt-12">
            <N8NUnificationDashboard />
          </div>
        )}

        {showTestDashboard && (
          <div className="mt-12">
            <EndToEndTestDashboard />
          </div>
        )}

        {showFidelityTestDashboard && (
          <div className="mt-12">
            <SystemFidelityTestDashboard />
          </div>
        )}

        {showDataSourceTest && (
          <div className="mt-12">
            <DataSourceIndicatorTest />
          </div>
        )}

        {showAutoStealthDashboard && (
          <div className="mt-12">
            <AutoStealthScrapingDashboard />
          </div>
        )}
      </main>
    </div>
