...
class ProjectGenerator {
    ...
    async generateReactFiles(projectPath, projectName, options) {
    ...
    async generateNextJSFiles(projectPath, projectName, options) {
        // Generate package.json
        const packageJson = {
            name: projectName,
            version: '1.0.0',
    ...
    async generateReactNativeFiles(projectPath, projectName, options) {
        // Generate package.json
        const packageJson = {
            name: projectName,
            version: '1.0.0',
    ...
    async generateUniversalFiles(projectPath, projectName, options) {
        // Generate package.json
        const packageJson = {
            name: projectName,
            version: '1.0.0',
    ...
    async generateReactSourceFiles(projectPath, projectName) {
...
<html lang="en">
  ...
  <body>
    ...
    async generateNextJSSourceFiles(projectPath, projectName) {
        const srcPath = path.join(projectPath, 'src');
...
export default function RootLayout({
  ...
  padding: 2rem;
    ...
    async generateReactNativeSourceFiles(projectPath, projectName) {
        const srcPath = path.join(projectPath, 'src');
        await fs.ensureDir(srcPath);
...
AppRegistry.registerComponent(appName, () => App);`;
    ...
    async generateUniversalSourceFiles(projectPath, projectName) {
        const srcPath = path.join(projectPath, 'src');
        await fs.ensureDir(srcPath);
...
export const View: React.FC<ViewProps> = ({ children, className, style }) => {
  return (
    ...
    async generateSharedFiles(projectPath, projectName, options) {
...
`;
    ...
    async generateAIIntegrationFiles(projectPath, projectName, options) {
...
`;
    ...
    async installDependencies(projectName) {
        const projectPath = path.join(process.cwd(), 'apps', projectName);
        try {
            await (0, execa_1.execa)('npm', ['install'], { cwd: projectPath });
        }
        catch (error) {
            console.error('Failed to install dependencies:', error);
        }
    }
    async generateInitialComponents(projectName, options) {
