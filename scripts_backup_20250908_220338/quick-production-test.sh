...
test_shell_generation() {
    printf '%s\n' "Testing shell script generation..."
    
    local test_dir="test-output"
    mkdir -p "$test_dir"
    
    # Generate a test script
    if ./scripts/production-shell-engine.sh generate "quick-test.sh" "basic" "$test_dir" >/dev/null 2>&1; then
...
test_error_handling() {
    ...
    # Test invalid input
    if ! ./scripts/production-shell-engine.sh generate "invalid" "basic" 2>/dev/null; then
...
test_performance() {
    ...
    if ./scripts/production-shell-engine.sh batch "$test_dir" >/dev/null 2>&1; then
