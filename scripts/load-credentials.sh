...
echo "🔐 Alex AI Universal Credentials Loader"
echo "======================================"
...
if [ -f ~/.zshrc ]; then
    ...
    echo "⚠️  ~/.zshrc not found, using environment variables"
fi
...
echo "📝 Creating .env files for monorepo apps..."
...
echo "✅ .env.local files created for all apps"
...
echo "✅ Global .env file created"

# Verify credentials are accessible
echo "🔍 Verifying credential accessibility..."
...
if [ -n "$N8N_URL" ] && [ -n "$N8N_API_KEY" ]; then
    echo "  ✅ N8N Federation Crew credentials available"
else
    echo "  ❌ N8N Federation Crew credentials missing"
fi

if [ -n "$OPENAI_API_KEY" ] || [ -n "$ANTHROPIC_API_KEY" ] || [ -n "$OPENROUTER_API_KEY" ]; then
    echo "  ✅ LLM API credentials available"
else
    echo "  ❌ LLM API credentials missing"
fi

echo ""
echo "🎉 Alex AI Universal Credentials Loader Complete!"
