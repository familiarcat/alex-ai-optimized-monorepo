...
test_shell_generation() {
    ...
    if ./scripts/deployment/general/consolidated_general.py generate "test-generated.sh" "basic" "$TEST_RESULTS_DIR"; then
...
# Test error handling
    ...
    if ! ./scripts/deployment/general/consolidated_general.py generate "invalid-name" "basic" 2>/dev/null; then
    ...
    if ! ./scripts/deployment/general/consolidated_general.py test "nonexistent.sh" 2>/dev/null; then
...
# Test performance
    ...
    if ./scripts/deployment/general/consolidated_general.py batch "$TEST_RESULTS_DIR" >/dev/null 2>&1; then
